        ...
        try {
            ...
            if (jsonObject.getString(KEY_SUCCESS).equals("true")) {
                ...
                for (int i = 0; i < dataArray.length(); i++) {
                    ...
                    preferenceHelper.putEmail(dataobj.getString(AndyConstants.Params.EMAIL));
                    preferenceHelper.putPhone(dataobj.getString(AndyConstants.Params.PHONE));
                    preferenceHelper.putAddress(dataobj.getString(AndyConstants.Params.ADDRESS));
                    preferenceHelper.putCnic(dataobj.getString(AndyConstants.Params.CNIC));
                    preferenceHelper.putPlaza(dataobj.getString(AndyConstants.Params.PLAZAID));
                    preferenceHelper.putBooth(dataobj.getString(AndyConstants.Params.BOOTHID));
                }
            }
        } catch (JSONException e) {
            e.printStackTrace();
