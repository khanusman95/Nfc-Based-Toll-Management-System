...
$link = mysqli_connect('localhost', 'root', 'root', 'toll',8889);
...
switch ($method) {
  case 'GET':
    $sql = "select * from `$table`".($key?" WHERE CNIC=$key":''); break;
  case 'PUT':
    $sql = "update `$table` set $set where CNIC=$key"; break;
  case 'POST':
    $sql = "insert into `$table` set $set"; break;
  case 'DELETE':
    $sql = "delete `$table` where CNIC=$key"; break;
}
